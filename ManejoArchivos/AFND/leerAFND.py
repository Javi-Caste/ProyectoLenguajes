...
# Patrones de expresiones regulares para la lectura del archivo
RE_NOMBRE = re.compile(r'^(?:NOMBRE-AFND|NOMBRE)\s*[:=]\s*(.+)$', re.IGNORECASE)
...
RE_TRANSICION = re.compile(r'^\s*([a-zA-Z0-9_]+)\s*,\s*([^,|\s]+)\s*,\s*([a-zA-Z0-9_|\s]+)\s*$')

# Funcion para procesar y cargar un AFND desde un archivo de texto
def cargarAFNDDesdeArchivo(rutaArchivo):
    ...
    for numLinea, linea in enumerate(lineas, 1):
        ...
        if not enSeccionTransiciones:
            ...
            if mTrans:
                qOrigen = mTrans.group(1)
                simbolo = mTrans.group(2)
                textoDestinos = mTrans.group(3)
                # Cada destino se guarda y se valida como una transicion individual.
                for destino in textoDestinos.split("|"):
                    destino = destino.strip()
                    if destino:
                        transicionesLista.append((qOrigen, simbolo, destino, numLinea))
            else:
                print(f"[ADVERTENCIA] Linea {numLinea}: Formato de transicion invalido -> '{lineaLimpia}'")

    # Creacion del objeto AFND
    nuevoAFND = AFND(nombre)
    nuevoAFND.definir_alfabeto(alfabetoLista)

    for est in estadosLista:
        esIni = (est == inicialStr)
        esFin = (est in finalesLista)
        nuevoAFND.agregar_nodo(est, es_inicial=esIni, es_final=esFin)

    # Insercion y validacion de transiciones
    for qOrig, sim, qDest, nLinea in transicionesLista:
        if qOrig not in nuevoAFND.nodos or qDest not in nuevoAFND.nodos:
            print(f"[ERROR] Linea {nLinea}: Estados '{qOrig}' o '{qDest}' no declarados en ESTADOS.")
            continue
        if sim not in nuevoAFND.alfabeto:
            print(f"[ERROR] Linea {nLinea}: Simbolo '{sim}' no pertenece al ALFABETO.")
            continue
        try:
            # El nodo conserva una lista de destinos para cada simbolo.
            nuevoAFND.agregar_transicion(qOrig, sim, qDest)
        except ValueError as err:
            print(f"[ERROR] Linea {nLinea}: {err}")
